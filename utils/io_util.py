...
def glob_imgs(path):
    imgs = []
    for ext in ['*.png', '*.jpg', '*.JPEG', '*.JPG', '*.bmp', '*.hdr']:
...
def load_gray(path, downscale=1):
    ...
    img = skimage.img_as_float32(img)
    # If as_gray is True, img_as_float32 WON'T be NORMALIZED
    img = img/255 
    if downscale != 1:
        img = rescale(img, 1./downscale, anti_aliasing=False, channel_axis=None)
...
def load_mask(path, downscale=1):
    ...
    if downscale != 1:
        alpha = rescale(alpha, 1./downscale, anti_aliasing=False, channel_axis=None)
    object_mask = alpha > 127.5

    return object_mask

def load_mask_u8(path, downscale=1):
...
def load_npy(path, downscale =1):
    npy_ = np.load(path)
    if downscale !=1:
        npy_ = rescale(npy_, 1./downscale, anti_aliasing=False, channel_axis=None) # should be [H,W], [H,W,1] WON'T WORK
    return npy_
...
def backup(backup_dir):
    ...
    subdirs_to_copy = ["", "dataio/", "models/", "tools/", "debug_tools/", "utils/", "models/frameworks/"]
...
def create_args_parser():
    ...
    parser.add_argument('--base_config', type=str, default=None, help='Directory of experiment to load.')
    return parser
