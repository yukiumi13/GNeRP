...
def main_function(args):
    ...
    if is_master():
        ...
        # Git Backup
        # repo = git.Repo('./')
        # log.info("=> Commit ID: {}".format(repo.heads[0].commit.hexsha))
        # with open(f'{exp_dir}/code.tar','wb') as f:
        #     repo.archive(f)
        # log.info('=> Git Archived')
                ...
                for (indices, model_input, ground_truth) in dataloader:
                    ...
                    if i_val > 0 and int_it % i_val == 1:
                        ...
                        with torch.no_grad():
                            ...
                            logger.add_imgs(to_img((rgb-target_rgb).abs()), 'val/rgb_error_map', it)
                            ...
                            if hasattr(trainer, 'val_pol') and render_kwargs_test['has_pol']:
                                ...
                                gt = {}
                                if 'mask_ignore' in val_gt:
                                    gt['mask_ignore'] = val_gt['mask_ignore'].to(device)
                                gt['AoP_map']= AoP_map
                                gt['DoP_map']= DoP_map
                                gt['rgb'] = target_rgb
                                gt['mask'] = mask
                                gt['aop_samples'] = aop_samples
                                trainer.val_pol(logger, ret, c2w, gt, to_img, it, render_kwargs_test)
                    ...
                    #-------------------
                    # delete autograd, see https://github.com/pytorch/pytorch/issues/82218
